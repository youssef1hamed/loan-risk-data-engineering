...
> ⚠️ The original dataset is in Parquet format and contains sensitive financial information.
...
- **Spark SQL** – Analytical querying and window functions
- **Google Colab** – Cloud execution environment

### **Data Processing**
- **PySpark ML** – Feature encoding and transformation utilities
- **Pandas** – Complementary data manipulation (if needed)
...
- Repartitioned data to match logical core count for performance optimization
...
- **Label Encoding** applied to: `emp_length`, `state`, `purpose` (using StringIndexer)
...
- Preserved encoding mappings for reproducibility
...
- **Sequential analysis**: Time gaps and amount differences between consecutive loans
...
- **Cross-verification**: Ensured SQL and DataFrame results matched for validation
...
- Organized output in structured directory format
...
- **Missing Value Analysis**: 
  ...
  - `emp_length` had 6.99% missing (imputed appropriately)
- **Grade Column**: Originally contained numeric values (1-34) but represented letter grades; discretized to first digit
- **Data Consistency**: Strong relationships observed between income, loan amount, and repayment likelihood
...
1. **Current Loans by Income & Employment**:
   ...
   - Employment length correlates with loan approval amounts
...
2. **Verification Status Impact**:
   ...
   - Data inconsistencies significantly affect model reliability if not handled properly
...
1. **Time Gaps Between Loans**:
   ...
   - Temporal patterns crucial for predicting repayment behavior
...
2. **State-Level Loan Amount Changes**:
   ...
   - Geographic factors significantly influence lending decisions
...
- **Income Distribution**: Wide range from low to high income brackets affecting loan terms
...
- **Cloud Integration**: Google Colab compatibility for easy deployment
...
2. **Model Readiness**:
   ...
   - Prepare data for multiple ML algorithms (classification, regression)

3. **Monitoring & Validation**:
   - Add data quality checks at each transformation stage
   - Implement schema validation
   - Create data drift detection mechanisms
   - Establish automated testing for pipeline components
